...
| :--- | :--- | :--- |
| `/steam` | `jogo` (String, obrigatório), `regiao` (Choice, opcional, padrão: `BR`) | Consulta o jogo na Steam API na região indicada (BR, US, PT, GB, JP, CA, AR) com autocomplete via cache TTL. Exibe embed estilizado (com destaque dourado para 100% OFF) e botão de monitoramento direto. Se houver histórico ou dados ITAD, anexa gráfico de tendência. |
| `/eshop` | `jogo` (String, obrigatório), `regiao` (Choice, opcional, padrão: `BR`) | Consulta o jogo na API da Nintendo eShop oficial do país indicado (com roteamento Algolia/Solr e fallbacks). Exibe valores monetários locais e botão de monitoramento. Se houver histórico, anexa gráfico de tendência. |
| `/comparar` | `plataforma` (Literal: `Steam`, `eShop`), `jogo` (String, obrigatório) | Dispara 5 consultas simultâneas em paralelo via `asyncio.gather` para **Brasil (BR / R$)**, **Estados Unidos (US / $)**, **Canadá (CA / CA$)**, **Portugal/Europa (PT / €)** e **Japão (JP / ¥)**, gerando uma matriz comparativa única com preços oficiais e badges de desconto. |
| `/historico` | `plataforma` (Literal: `Steam`, `eShop`), `jogo` (String, obrigatório), `regiao` (Choice, opcional, padrão: `BR`) | Retorna a análise temporal detalhada e o gráfico de variação de preço garantido para o título especificado na região selecionada (sintetizado via ITAD se necessário). |
...
| `/gratis` | Nenhum | Rastreador sob demanda de Giveaways e promoções temporárias de 100% OFF (*Free to Keep*) na Steam. Exibe valor original riscado, data de término do resgate e botões de link direto para adicionar à biblioteca. |
| `/canal_gratis` | `canal` (Canal de texto, opcional) | Define ou remove o canal de avisos automáticos de jogos 100% grátis na Steam para o servidor (notificações automáticas a cada 1 hora, sem `@everyone`). |
